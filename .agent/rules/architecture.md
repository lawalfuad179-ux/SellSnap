...
SellSnap is a Next.js application using the App Router, written in TypeScript, backed by PostgreSQL through Prisma. Payments run through Flutterwave. Styling is handled by CSS Modules with design tokens. There is no separate backend service. Everything lives in the Next.js app, using server components, server actions, and route handlers.
